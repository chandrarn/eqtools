...
class EqdskReader(Equilibrium):
    ...
    def __init__(self,
        ...
        with open(self._gfilename,'r') as gfile:
            ...
            self._time = np.array([float(time)*timeConvertDict[tunits]]) # returns time in seconds as array
            ...
            # construct EFIT grid
            self._rGrid = np.linspace(rgrid0,rgrid0 + xdim,nw)
            self._zGrid = np.linspace(zmid - zdim/2.0,zmid + zdim/2.0,nh)
            ...
            self._rmag = np.array([float(line[0])])
            self._zmag = np.array([float(line[1])])
            self._defaultUnits['_rmag'] = 'm'
            self._defaultUnits['_zmag'] = 'm'
            self._psiAxis = np.array([float(line[2])])
            self._psiLCFS = np.array([float(line[3])])
            self._bzero = np.array([float(line[4])])
            ...
            line = next(reader)[0]
            line = re.findall('-?\d\.\d*[eE][-+]\d*',line)
            self._IpCalc = np.array([float(line[0])])
            ...
            self._fpol = []
            for i in np.arange(nrows,dtype=int):
                line = next(reader)[0]
                line = re.findall('-?\d\.\d*[eE][-+]\d*',line)
                for val in line:
                    self._fpol.append(float(val))
            self._fpol = np.array(self._fpol).reshape((1,nw))
            ...
            self._fluxPres = np.array(self._fluxPres).reshape((1,nw))
            ...
            self._ffprim = np.array(self._ffprim).reshape((1,nw))
            ...
            self._pprime = np.array(self._pprime).reshape((1,nw))
            ...
            self._psiRZ = np.array(psis).reshape((1,nh,nw),order='C')
            ...
            self._qpsi = np.array(self._qpsi).reshape((1,nw))
            ...
            bbbs = np.array(bbbs).reshape((2,nbbbs),order='F')
            ...
            lim = np.array(lim).reshape((2,limitr),order='F')
            self._xlim = lim[0,:]
            self._ylim = lim[1,:]
            ...
            try:
                ...
                if kvtor > 0:
                    ...
                    self._presw = np.array(self._presw).reshape((nw,1))
                    self._preswp = []
                    for i in np.arange(nrows,dtype=int):
                        line = next(reader)[0]
                        line = re.findall('-?\d.\d*[eE][-+]\d*',line)
                    ...
                    self._preswp = np.array(self._preswp).reshape((1,nw))
                else:
                    self._presw = scipy.atleast_2d(np.array([0]))
                    self._preswp = scipy.atleast_2d(np.array([0]))
                ...
                if nmass > 0:
                    ...
                    self._dmion = np.array(self._dmion).reshape((1,nw))
                else:
                    self._dmion = scipy.atleast_2d(np.array([0]))
                ...
                self._rhovn = np.array(self._rhovn).reshape((1,nw))
                ...
                if keecur > 0:
                    ...
                    self._workk = np.array(self._workk).reshape((1,nw))
                else:
                    self._workk = scipy.atleast_2d(np.array([0]))
            except:
                self._presw = scipy.atleast_2d(np.array([0]))
                self._preswp = scipy.atleast_2d(np.array([0]))
                self._rhovn = scipy.atleast_2d(np.array([0]))
                self._dmion = scipy.atleast_2d(np.array([0]))
                self._workk = scipy.atleast_2d(np.array([0]))
    ...
    def readAFile(self,afile):
        ...
        try:
            ...
            # fields
            self._btaxp = np.array([afr.btaxp])
            self._btaxv = np.array([afr.btaxv])
            self._bpolav = np.array([afr.bpolav])

            # currents
            self._IpMeas = np.array([afr.pasmat])

            # safety factor parameters
            self._q0 = np.array([afr.qqmin])
            self._q95 = np.array([afr.qpsib])
            self._qLCFS = np.array([afr.qout])
            self._rq1 = np.array([afr.aaq1])
            self._rq2 = np.array([afr.aaq2])
            self._rq3 = np.array([afr.aaq3])

            # shaping parameters
            self._kappa = np.array([afr.eout])
            self._dupper = np.array([afr.doutu])
            self._dlower = np.array([afr.doutl])

            # dimensional geometry parameters
            self._aLCFS = np.array([afr.aout])
            self._areaLCFS = np.array([afr.areao])
            self._RmidLCFS = np.array([afr.rmidout])

            # calc. normalized pressure values
            self._betat = np.array([afr.betat])
            self._betap = np.array([afr.betap])
            self._Li = np.array([afr.ali])

            # diamagnetic measurements
            self._diamag = np.array([afr.diamag])
            self._betatd = np.array([afr.betatd])
            self._betapd = np.array([afr.betapd])
            self._WDiamag = np.array([afr.wplasmd])
            self._tauDiamag = np.array([afr.taudia])

            # calculated energy
            self._WMHD = np.array([afr.wplasm])
            self._tauMHD = np.array([afr.taumhd])
            self._Pinj = np.array([afr.pbinj])
            self._Wbdot = np.array([afr.wbdot])
            self._Wpdot = np.array([afr.wpdot])

            # fitting parameters
            self._volLCFS = np.array([afr.vout])
    ...
    def getFluxAxis(self):
        ...
        return -1. * self.getCurrentSign() * np.array(self._psiAxis)
    ...
    def getFluxLCFS(self):
        ...
        """
        # scale by current sign for consistency with sign of psiRZ.
        return -1 * self.getCurrentSign() * np.array(self._psiLCFS)
    ...
    def remapLCFS(self,mask=False):
        ...
        RLCFS = np.array(RLCFS)
        ZLCFS = np.array(ZLCFS)
        
        # generate masking array
        if mask:
            maskarr = np.array([False for i in range(len(RLCFS))])
