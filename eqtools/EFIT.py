...
try:
    ...
    import MDSplus # Reverting to older MDS-style. Could be accessed through mdsthin compatibility mode as well
...
class EFITTree(Equilibrium):
    ...
    def __init__(self, shot, tree, root, length_unit='m', gfile = 'g_eqdsk', 
        ...
        #self._MDSTree.openTree(self._tree,self._shot)
    ...
    def remapLCFS(self,mask=False):
        ...
        for i in range(nt):
            ...
            for path in paths:
                ...
                RLCFS_frame.append(np.nan)
                ZLCFS_frame.append(np.nan)
            RLCFS_frame = np.array(RLCFS_frame)
            ZLCFS_frame = np.array(ZLCFS_frame)

            # generate masking array to vessel
            if mask:
                maskarr = np.array([False for i in range(len(RLCFS_frame))])
        ...
        RLCFS = np.zeros((nt,maxlen))
        ZLCFS = np.zeros((nt,maxlen))
        for i in range(nt):
    ...
    def getF(self):
        ...
        if self._fpol is None:
            try:
                ...
                self._defaultUnits['_fpol'] = self._defaultUnits['_fpol'] = fNode.units
            except TreeException:
                raise ValueError('data retrieval failed.')
